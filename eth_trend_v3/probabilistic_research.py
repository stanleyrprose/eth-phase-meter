...
class ModelArtifact:
    ...
    def to_dict(self):
        return {"features": self.features, "mean": self.mean, "scale": self.scale, "coef": self.coef, "intercept": self.intercept, "interactions": [list(x) for x in self.interactions], "model_type": self.model_type}
...
def predict_artifact(artifact: ModelArtifact | Mapping[str, Any], rows, base_features: list[str] | None = None):
    a = artifact if isinstance(artifact, ModelArtifact) else ModelArtifact(
        features=list(artifact["features"]), mean=list(artifact["mean"]), scale=list(artifact["scale"]), coef=list(artifact["coef"]), intercept=float(artifact["intercept"]), interactions=[tuple(x) for x in artifact.get("interactions", [])], model_type=artifact.get("model_type", "logistic")
    )
...
def evaluate_logistic(folds, features: list[str], *, horizon_bars: int, baseline_spec: BaselineSpec | None = None, interactions: list[tuple[str, str]] | None = None, C: float = 1.0, bootstrap_reps: int = 500):
    ...
    ys, ps, bs, fold_metrics, oos_predictions = [], [], [], [], []
    for fold_no, fold in enumerate(folds, start=1):
        ...
        ys.extend(y.tolist()); ps.extend(p.tolist()); bs.extend(bp.tolist())
        fold_metrics.append({"brier": brier(y, p), "baseline_brier": brier(y, bp), "n": len(y)})
        for i, row in enumerate(kept):
            oos_predictions.append({"key": _row_key(row, f"fold{fold_no}-{i}"), "fold": fold_no, "target_up": int(y[i]), "probability": float(p[i]), "baseline_probability": float(bp[i])})
    if not ys:
        return {"available": False, "reason": "NO_VALID_FOLDS"}
    y, p, bp = np.asarray(ys), np.asarray(ps), np.asarray(bs)
    ci = moving_block_delta_brier_ci(y, p, bp, horizon_bars, reps=bootstrap_reps)
    sensitivity = moving_block_sensitivity(y, p, bp, horizon_bars, reps=bootstrap_reps)
    return {
        "available": True,
        "features": features,
        "interactions": [list(x) for x in interactions],
        "metrics": {"brier": brier(y, p), "baseline_brier": brier(y, bp), "brier_skill": brier_skill_score(y, p, bp), "log_loss": log_loss(y, p), "calibration_error": calibration_error(y, p), "delta_brier_ci": ci, "block_sensitivity": sensitivity, "oos_n": len(y), "folds": fold_metrics},
        "oos_predictions": oos_predictions,
        "passes_incremental_gate": bool(brier(y, p) < brier(y, bp) and sensitivity.get("stable_positive")),
    }
...
def controlled_interactions(features: list[str], allowed_pairs: list[tuple[str, str]] | None = None, max_interactions: int = 20):
    ...
    return pairs[:max_interactions] if len(pairs) > max_interactions else pairs
