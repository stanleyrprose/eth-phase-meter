...
def calibration_error(y, p, bins: int = 10) -> float | None:
    ...
    for lo, hi in zip(edges[:-1], edges[1:]):
        ...
        if mask.any():
            k = int(mask.sum())
            total += k * abs(float(p[mask].mean() - y[mask].mean()))
            n += k
...
def moving_block_delta_brier_ci(y, candidate, baseline, block: int, reps: int = 1000, seed: int = 17):
    ...
    if len(y) == 0 or len(y) != len(c) or len(y) != len(b):
        return None
    block = max(1, min(int(block), len(y)))
    ...
    for _ in range(reps):
        ...
        while len(idx) < len(y):
            ...
            idx.extend(range(s, min(s + block, len(y))))
...
def moving_block_sensitivity(
    ...
    *,
    ...
    for offset, multiplier in enumerate(multipliers):
        block = max(1, int(round(float(horizon_bars) * float(multiplier))))
        key = f"{float(multiplier):g}H"
        results[key] = moving_block_delta_brier_ci(
            y,
            candidate,
            baseline,
            block,
    ...
    valid = [result for result in results.values() if result is not None]
    stable_positive = bool(valid) and len(valid) == len(results) and all(result.get("low", 0) > 0 for result in valid)
    return {
        "results": results,
        "stable_positive": stable_positive,
        "rule": "Incremental evidence is robust only when the 95% lower bound is positive at 0.5H, 1.0H, and 1.5H block lengths.",
    }
...
def effective_sample_diagnostic(n: int, horizon_bars: int) -> dict:
    ...
    return {
        ...
        "conservative_nonoverlap_n": int(n // max(1, horizon_bars)),
        "kind": "DIAGNOSTIC",
    }
