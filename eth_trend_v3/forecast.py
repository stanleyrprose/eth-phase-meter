...
MODEL_LADDER=[
 ...
 ('plus-capital-flow',['trend','crowding','volatility_risk','capital_flow']),
 ('plus-structural',['trend','crowding','volatility_risk','capital_flow','structural_supply']),
 ('plus-valuation',['trend','crowding','volatility_risk','capital_flow','structural_supply','valuation']),
]
...
def _matrix(rows,features):
    ...
    for r in rows:
        ...
        usable.append(r)
    X=np.asarray([[r[f] for f in features] for r in usable],dtype=float) if features else np.empty((len(usable),0))
    y=np.asarray([r['target_up'] for r in usable],dtype=int)
    return usable,X,y
...
def expanding_walk_forward(rows,features=None,min_train=120,test_size=24):
    ...
    if not features:
        ...
        for start in range(min_train,len(y),test_size):
            ...
            if len(np.unique(y[:fit_end]))<2: continue
            ...
            if len(np.unique(y[fit_end:start]))<2: continue
    ...
    metrics['passes_baseline_gate']=bool(features and metrics['brier_lift']>0 and metrics['oos_n']>=60)
...
def evaluate_model_ladder(rows):
    ...
    for name,features in MODEL_LADDER:
        r=expanding_walk_forward(rows,features); m=r.get('metrics') or {}; b=m.get('brier'); incremental=(previous_brier-b) if previous_brier is not None and b is not None else None
        entry={'name':name,'features':features,'result':r,'incremental_brier':incremental}; results.append(entry)
        if name=='base-rate':
            if b is not None: previous_brier=b
            continue
        if r.get('available') and m.get('passes_baseline_gate') and (incremental is None or incremental>0):
            selected=entry
            previous_brier=b
        elif selected is not None:
            # Kill additional complexity once it fails to add stable Brier improvement.
            break
