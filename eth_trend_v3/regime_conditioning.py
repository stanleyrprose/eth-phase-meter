...
def align_states(reference_profiles:list[dict], new_profiles:list[dict], keys=("mean_return","volatility")) -> dict[int,int]:
    ...
    for perm in itertools.permutations(range(k)):
        ...
        for i,j in enumerate(perm):
            for key in keys:
                a=float(reference_profiles[i].get(key,0.0)); b=float(new_profiles[j].get(key,0.0)); cost+=(a-b)**2
...
def regime_latency(regimes:list[str], stable_flags:list[bool])->dict:
    ...
    for i,(r,stable) in enumerate(zip(regimes,stable_flags)):
        if r!=last and pending is None: pending=i
        if pending is not None and stable and r!=last: delays.append(i-pending); last=r; pending=None
