...
def validate_production_summary(
    ...
    *,
    ...
    if notification_configured is False:
        ...
        if isinstance(notification_status, Mapping) and "forecast_summary" in notification_status:
            item = notification_status.get("forecast_summary")
            status = item.get("status") if isinstance(item, Mapping) else None
            if status != "SENT":
                errors.append("TELEGRAM_FORECAST_SUMMARY_NOT_CONFIRMED")
        else:
