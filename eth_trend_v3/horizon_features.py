...
class FeatureMeta:
    feature_name:str; feature_version:str; source:str; formula:str; lookback:str; timestamp_semantics:str; source_delay:str; missing_policy:str; information_cluster:str; horizon_relevance:str
    def to_dict(self): return asdict(self)


def _base_frame(candles:pd.DataFrame)->pd.DataFrame:
    df=candles.copy(); df["timestamp"]=pd.to_datetime(df["timestamp"],utc=True); df=df.sort_values("timestamp").drop_duplicates("timestamp",keep="last")
    df["close"]=pd.to_numeric(df["close"],errors="coerce"); df["volume"]=pd.to_numeric(df["volume"],errors="coerce")
    df["ret_4h"]=np.log(df["close"]/df["close"].shift(1)); return df


def build_horizon_features(candles:pd.DataFrame, horizon:str, macro:pd.DataFrame|None=None)->pd.DataFrame:
    df=_base_frame(candles); windows={"3d":[6,18],"7d":[6,18,42],"30d":[18,42,84,180]}[horizon]
    for w in windows:
    ...
    if horizon=="30d":
        rollmax=df["close"].rolling(180,min_periods=180).max(); df["drawdown_30d"]=df["close"]/rollmax-1
    if macro is not None and len(macro):
        m=macro.copy(); m["available_at"]=pd.to_datetime(m["available_at"],utc=True); m=m.sort_values("available_at")
...
def correlation_audit(df:pd.DataFrame, features:list[str], threshold:float=.8)->dict:
    corr=df[features].corr(); pairs=[]
    for i,a in enumerate(features):
        for b in features[i+1:]:
            v=corr.loc[a,b]
            if pd.notna(v) and abs(v)>=threshold: pairs.append({"a":a,"b":b,"r":float(v)})
