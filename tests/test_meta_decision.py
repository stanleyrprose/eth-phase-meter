...
def _ta(decision="HOLD", symbol="ETH-USD"):
    return {
...
def test_current_like_state_stays_hold_without_inventing_probability():
    ...
    result = evaluate_meta_decision(monitor, _ta("HOLD"))
...
def test_add_requires_cross_system_and_multitimeframe_confirmation():
    ...
    result = evaluate_meta_decision(monitor, _ta("BUY"))
...
def test_reduce_requires_confirmed_bearish_deterioration():
    ...
    result = evaluate_meta_decision(monitor, _ta("SELL"))
...
def test_asset_mismatch_fails_closed_to_avoid():
    ...
    result = evaluate_meta_decision(monitor, _ta("BUY", symbol="BTC-USD"))
    assert result["recommendation"] == "AVOID"
