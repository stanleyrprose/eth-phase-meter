...
- R2 Tactical Outcome Evaluation is implemented as a prospective research-only layer: immutable in-SLA 1H tactical events are stored separately from append-only +1H/+4H/+12H/+24H settlements; reports cover requested state/gate/±20/direction-change cohorts, exclude degraded events from primary descriptive summaries, and cannot auto-retune thresholds, promote models, or execute trades.
- Daily R2 Tactical Research Readiness monitor reports operational health and evidence accumulation separately: settlement completeness for +1H/+4H/+12H/+24H, event/outcome counts, cohort coverage, duplicate identities, settlement lag, missing target PIT, incomplete paths, and a concise Telegram summary. It deliberately has no automatic statistical-sufficiency threshold.
